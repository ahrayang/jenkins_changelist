...
def run_command(cmd):
    logging.info(f"[백그라운드 CMD 실행 시작]: {cmd}") 
    try:
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True, check=True)
        logging.info(f"[CMD 실행 결과 안내]:\n{result.stdout}")
...
def get_changes(depot, since, until):
    cmd = f'p4 changes -s submitted {depot}@{since},{until}'
    logging.info(f"cmd 전송 로그: {cmd}")
...
def parse_describe(change_num):
    ...
    if jira_match:
        info["jira_url"] = jira_match.group(1)
    else:
        info["jira_url"] = ""
...
def append_to_excel(data_list, excel_file="build_history.xlsx"):
    ...
    if os.path.exists(excel_file):
        try:
            ...
            logging.error("Excel 파일 읽기 오류:", e)
            df_total = df_new
    else:
        df_total = df_new
    try:
